...
def get_higher_card(x, y):
    initial_ranking = ["A", "K", "Q", "T", "9", "8", "7", "6", "5", "4", "3", "2", "J"]
...
def find_joker_substitute(x):
    if "J" not in x or x == "JJJJJ":
        return x

    x2 = {xi: x.count(xi) for xi in set(x) if xi != "J"}
    x2 = sorted(x2.keys(), key=x2.get, reverse=True)
    
    return x.replace("J", x2[0])

def second_is_greater(x, y):
    x2 = find_joker_substitute(x)
    y2 = find_joker_substitute(y)

    x_d = sorted([x2.count(xi) for xi in set(x2)])
    y_d = sorted([y2.count(yi) for yi in set(y2)])

    # five of a kind
    if len(x_d) == 1 and len(y_d) == 1:
